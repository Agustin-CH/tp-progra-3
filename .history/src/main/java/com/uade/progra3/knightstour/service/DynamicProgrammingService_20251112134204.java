 ...
 * 
 * Enfoque: Usamos memoización con una tabla:
...
public class DynamicProgrammingService {
    ...
    private int solveDP(int row, int col, int movesLeft) {
        ...
        if (movesLeft == 0) {
            memo[row][col][0] = pointsBoard[row][col];
        ...
        while (movesLeft > 0) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (isValid(nextRow, nextCol)) {
                    ...
                    // Solo considerar si el valor fue calculado (no es -1)
            ...
            // Si no se encontró siguiente movimiento válido, hay un error en la lógica
            if (bestRow == -1) {
                System.err.println("Error: No se pudo reconstruir el camino completo. " +
