...
public class DynamicProgrammingService {
    ...
    // Usamos un valor muy negativo para representar estados imposibles cuando se requiere
    // exactamente 'm' movimientos y no hay vecinos válidos que permitan completar el recorrido.
    ...
    public SolutionResult solve(int boardSize, int startRow, int startCol, 
        ...
        int actualMoves = Math.max(0, bestPath.size() - 1);
        boolean exactKReached = (maxScore != NEG_INF) && (actualMoves == maxMoves);
        ...
        return SolutionResult.builder()
            ...
            .message(exactKReached
                ? String.format("Puntaje máximo alcanzado: %d puntos en %d movimientos", maxScore, maxMoves)
                : String.format("No existe recorrido de exactamente %d movimientos desde la posición inicial. Se alcanzaron %d movimientos.", 
    ...
    private int solveDP(int row, int col, int movesLeft) {
        ...
        // Para "exactamente m" movimientos, si no hay vecinos válidos debemos marcar imposible.
        int maxFuturePoints = NEG_INF;
        ...
        // Si no hubo ningún vecino válido que permita completar exactamente 'movesLeft-1' movimientos,
        // este estado es imposible.
        if (maxFuturePoints == NEG_INF) {
            memo[row][col][movesLeft] = NEG_INF;
            return memo[row][col][movesLeft];
        }

        // Memoizar y retornar puntaje actual + mejor continuación
        ...
        while (movesLeft > 0) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (isValid(nextRow, nextCol)) {
                    ...
                    // Solo consideramos vecinos que permitan completar exactamente movesLeft-1
                    if (value != NEG_INF && value > bestValue) {
                        bestValue = value;
                        bestRow = nextRow;
                        bestCol = nextCol;
                    }
