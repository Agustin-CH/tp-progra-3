...
public class BacktrackingService {
    ...
    private int maxSolutionsForCurrentBoard;  // Límite dinámico según tamaño del tablero
    ...
    public SolutionResult solve(int boardSize, int startRow, int startCol) {
        ...
        // Para tableros grandes (>8), solo buscar 1 solución para mejor rendimiento
        maxSolutionsForCurrentBoard = (boardSize > 8) ? 1 : MAX_SOLUTIONS;
        
        // Buscar soluciones (1 para tableros grandes, 6 para pequeños)
        ...
        // Solo para tableros <= 8
        List<int[][]> alternatives = new ArrayList<>();
        if (boardSize <= 8 && allSolutions.size() > 1) {
