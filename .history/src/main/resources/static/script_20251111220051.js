...
loadExampleSolutions();

async function loadExampleSolutions() {
    // Cargar soluciones de ejemplo dinámicamente desde el backend
    try {
        const response = await fetch(`${API_URL}/backtracking?boardSize=5&startRow=0&startCol=0`);
        const data = await response.json();
        
        if (data.success && data.alternativeSolutions && data.alternativeSolutions.length > 0) {
            renderExampleSolutions(data.alternativeSolutions);
        }
    } catch (error) {
...
function renderExampleSolutions(solutions) {
    ...
    solutions.forEach((board, index) => {
        ...
        miniBoard.title = `Solución ${index + 1}`;
...
function displayResults(data) {
    ...
    if (data.alternativeSolutions && data.alternativeSolutions.length > 0) {
        html += `
            <div style="margin-top: 15px; padding-top: 15px; border-top: 1px solid #ddd;">
                <p><strong>📋 Caminos alternativos encontrados:</strong> ${data.alternativeSolutions.length}</p>
                <p style="font-size: 12px; color: #666;">(Parte opcional del TP: mostrar todos los caminos posibles)</p>
                <div id="alternativeSolutions" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(100px, 1fr)); gap: 10px; margin-top: 10px;">
                </div>
            </div>
        `;
    }
    ...
    // Renderizar mini tableros alternativos
    if (data.alternativeSolutions && data.alternativeSolutions.length > 0) {
        renderAlternativeSolutions(data.alternativeSolutions);
    }
}
...
function renderAlternativeSolutions(solutions) {
    ...
    solutions.forEach((board, index) => {
        ...
        miniBoard.title = `Solución alternativa ${index + 1} - Click para ver`;
        miniBoard.onclick = () => {
            renderBoard(board);
        ...
        label.style.cssText = 'text-align: center; font-size: 11px; margin-top: 5px; font-weight: 500;';
        
        const wrapper = document.createElement('div');
        wrapper.appendChild(miniBoard);
...
function renderAlternativeSolutions(solutions) {
    ...
    solutions.forEach((board, index) => {
        const miniBoard = document.createElement('div');
        miniBoard.className = 'mini-board';
        miniBoard.title = `Solución alternativa ${index + 1}`;
        miniBoard.onclick = () => {
            renderBoard(board);
            currentSolution.board = board;
            currentSolution.path = reconstructPathFromBoard(board);
        ...
        const size = board.length;
        const cellSize = 100 / size;  // Tamaño relativo
