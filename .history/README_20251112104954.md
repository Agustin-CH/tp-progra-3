# 🐴 Knight's Tour Problem - Trabajo Práctico Programación 3

## 📋 Descripción del Proyecto

Este proyecto implementa **tres soluciones diferentes** al problema clásico del **Knight's Tour** (Recorrido del Caballo) en Java, con una interfaz web interactiva para visualizar y comparar los algoritmos.

**Trabajo Práctico Obligatorio - Programación 3 - UADE 2025**

### Objetivos del Proyecto

1. **Implementar y comparar diferentes técnicas algorítmicas:**
   - Backtracking (búsqueda exhaustiva)
   - Heurística Greedy (Regla de Warnsdorff)
   - Programación Dinámica (maximización de puntos)

2. **Analizar complejidad temporal y espacial** de cada algoritmo

3. **Crear una aplicación web funcional** que demuestre los conceptos de forma visual e interactiva

## Tecnologías Utilizadas

### Backend
- **Java 17**
- **Spring Boot 3.1.5** - Framework web
- **Maven** - Gestión de dependencias

### Frontend
- **HTML5**
- **CSS3** (diseño responsive y moderno)
- **JavaScript Vanilla** (sin frameworks)

## Estructura del Proyecto

```
Tp-progra-iii/
├── src/
│   ├── main/
│   │   ├── java/com/uade/progra3/knightstour/
│   │   │   ├── KnightsTourApplication.java      # Clase principal
│   │   │   ├── controller/
│   │   │   │   └── KnightsTourController.java   # REST API
│   │   │   ├── service/
│   │   │   │   ├── BacktrackingService.java     # Algoritmo Backtracking
│   │   │   │   ├── WarnsdorffService.java       # Algoritmo Greedy
│   │   │   │   └── DynamicProgrammingService.java # Algoritmo PD
│   │   │   └── model/
│   │   │       ├── Position.java
│   │   │       ├── SolutionResult.java
│   │   │       ├── ComplexityAnalysis.java
│   │   │       └── DynamicProgrammingRequest.java
│   │   └── resources/
│   │       ├── static/
│   │       │   ├── index.html                   # Frontend
│   │       │   ├── styles.css
│   │       │   └── script.js
│   │       └── application.properties
├── pom.xml                                       # Configuración Maven
└── README.md
```

## 🛠️ Instalación y Ejecución

### Prerequisitos

- **Java 17 o superior** ([Descargar aquí](https://adoptium.net/))
- **Maven 3.6+** (o usar el wrapper incluido)
- **Navegador web moderno**

### Pasos para ejecutar el proyecto

1. **Clonar o descargar el repositorio**

```bash
cd Tp-progra-iii
```

2. **Compilar el proyecto**

```bash
mvn clean install
```

3. **Ejecutar la aplicación**

```bash
mvn spring-boot:run
```

O alternativamente:

```bash
java -jar target/knights-tour-1.0.0.jar
```

4. **Abrir en el navegador**

```
http://localhost:8080
```

La aplicación estará corriendo en el puerto **8080**.

## 🎮 Cómo Usar la Aplicación

### Interfaz Web

1. **Seleccionar un algoritmo:**
   - Backtracking
   - Warnsdorff (Greedy)
   - Programación Dinámica
   - Comparar Algoritmos

2. **Configurar parámetros:**
   - Tamaño del tablero (4×4 hasta 12×12)
   - Posición inicial del caballo
   - Para PD: número de movimientos

3. **Hacer clic en "Resolver"**

4. **Visualizar resultados:**
   - Tablero con el recorrido numerado
   - Métricas de rendimiento
   - Análisis de complejidad

5. **Opcional: Animar la solución** paso a paso

### API REST

El backend expone los siguientes endpoints:

#### 1. Backtracking
```bash
GET http://localhost:8080/api/knights-tour/backtracking?boardSize=5&startRow=0&startCol=0
```

#### 2. Warnsdorff
```bash
GET http://localhost:8080/api/knights-tour/warnsdorff?boardSize=8&startRow=0&startCol=0
```

#### 3. Programación Dinámica
```bash
POST http://localhost:8080/api/knights-tour/dynamic-programming
Content-Type: application/json

{
  "boardSize": 8,
  "startRow": 0,
  "startCol": 0,
  "maxMoves": 10,
  "pointsBoard": null
}
```

#### 4. Comparar Algoritmos
```bash
GET http://localhost:8080/api/knights-tour/compare?boardSize=6&startRow=0&startCol=0
```

#### 5. Generar Tablero de Puntos
```bash
GET http://localhost:8080/api/knights-tour/generate-points-board?boardSize=8
```

## 🧮 Análisis de Algoritmos

### 1️⃣ Backtracking

**Descripción:** Explora exhaustivamente todos los caminos posibles de forma recursiva. Si un camino no lleva a solución, retrocede (backtrack) y prueba otra ruta.

**Complejidad:**
- **Temporal:** O(8^(n²)) - Exponencial
- **Espacial:** O(n²)

**Ventajas:**
- ✅ Garantiza encontrar solución si existe
- ✅ Encuentra todas las soluciones posibles

**Desventajas:**
- ❌ Muy lento para tableros grandes (n > 6)
- ❌ Explora muchos caminos inútiles

**Caso de uso:** Tableros pequeños (≤ 6×6) donde se necesita garantía de solución.

---

### 2️⃣ Heurística de Warnsdorff (Greedy)

**Descripción:** En cada paso, elige la casilla con menor número de movimientos disponibles (grado de accesibilidad). Esta estrategia "greedy" evita quedar atrapado en esquinas.

**Complejidad:**
- **Temporal:** O(n²) - Lineal en número de casillas
- **Espacial:** O(n²)

**Ventajas:**
- ✅ Muy rápido (milisegundos incluso para n=20)
- ✅ Alta tasa de éxito (~90% en tableros grandes)
- ✅ No requiere backtracking

**Desventajas:**
- ❌ No garantiza solución en todos los casos
- ❌ Puede fallar en casos específicos

**Caso de uso:** Tableros grandes (≥ 8×8) donde se prioriza velocidad.

---

### 3️⃣ Programación Dinámica

**Descripción:** Variante del problema donde se maximiza la suma de puntos visitados en exactamente K movimientos. Usa memoización para evitar recalcular subproblemas.

**Complejidad:**
- **Temporal:** O(n² × k) donde k = número de movimientos
- **Espacial:** O(n² × k)

**Ventajas:**
- ✅ Solución óptima para maximización de puntos
- ✅ Evita recalcular subproblemas
- ✅ Eficiente para valores moderados de k

**Desventajas:**
- ❌ Alto uso de memoria para k grande
- ❌ Resuelve un problema diferente al Knight's Tour clásico

**Caso de uso:** Problemas de optimización con número limitado de movimientos.

---

## 📊 Tabla Comparativa

| Algoritmo | Complejidad Temporal | Complejidad Espacial | Velocidad (8×8) | Garantía de Solución |
|-----------|---------------------|---------------------|----------------|---------------------|
| **Backtracking** | O(8^(n²)) | O(n²) | ~5000ms | ✅ Sí |
| **Warnsdorff** | O(n²) | O(n²) | ~5ms | ❌ No (alta probabilidad) |
| **Prog. Dinámica** | O(n² × k) | O(n² × k) | ~50ms (k=10) | ✅ Sí (para maximización) |

## 🎓 Conceptos de Programación 3 Aplicados

### Técnicas Algorítmicas

1. **Backtracking (Vuelta Atrás)**
   - Recursión
   - Poda de árbol de búsqueda
   - Exploración exhaustiva

2. **Greedy (Algoritmos Voraces)**
   - Elección localmente óptima
   - Heurísticas
   - No siempre garantiza óptimo global

3. **Programación Dinámica**
   - Subestructura óptima
   - Superposición de subproblemas
   - Memoización
   - Bottom-up vs Top-down

### Estructuras de Datos

- Arrays multidimensionales (tablero)
- Listas (camino del caballo)
- Tablas de memoización 3D

### Análisis de Complejidad

- Notación Big-O
- Complejidad temporal vs espacial
- Trade-offs entre algoritmos

## 📝 Ejemplos de Ejecución

### Ejemplo 1: Backtracking en 5×5

```
Entrada: boardSize=5, startRow=0, startCol=0
Salida:
┌────┬────┬────┬────┬────┐
│  0 │  3 │ 14 │  9 │ 20 │
├────┼────┼────┼────┼────┤
│ 13 │  8 │  1 │  4 │ 15 │
├────┼────┼────┼────┼────┤
│  2 │ 23 │ 12 │ 19 │ 10 │
├────┼────┼────┼────┼────┤
│  7 │ 18 │ 21 │ 16 │  5 │
├────┼────┼────┼────┼────┤
│ 22 │ 11 │  6 │ 24 │ 17 │
└────┴────┴────┴────┴────┘

Tiempo: 2847ms
Pasos explorados: 1,234,567
```

### Ejemplo 2: Warnsdorff en 8×8

```
Entrada: boardSize=8, startRow=0, startCol=0
Salida: [Tablero completo con recorrido]
Tiempo: 3ms
Pasos explorados: 64
```

## 🐛 Troubleshooting

### Error: Puerto 8080 en uso

```bash
# En Mac/Linux
lsof -ti:8080 | xargs kill -9

# En Windows
netstat -ano | findstr :8080
taskkill /PID [PID] /F
```

### Error: Java no encontrado

Verificar instalación:
```bash
java -version
```

### Error: Maven no encontrado

Usar el wrapper de Maven incluido:
```bash
./mvnw spring-boot:run    # Mac/Linux
mvnw.cmd spring-boot:run  # Windows
```

## 📚 Referencias

- **Knight's Tour Problem**: [Wikipedia](https://en.wikipedia.org/wiki/Knight%27s_tour)
- **Warnsdorff's Rule**: [GeeksforGeeks](https://www.geeksforgeeks.org/warnsdorffs-algorithm-knights-tour-problem/)
- **Backtracking**: [Introduction to Algorithms - Cormen et al.]
- **Dynamic Programming**: [CLRS Chapter 15]

## 👥 Autores

Trabajo Práctico - Programación 3 - UADE 2025

## 📄 Licencia

Este proyecto es material educativo para el curso de Programación 3.

---

## 🎯 Preguntas Frecuentes del Coloquio

### 1. ¿Cómo funciona DFS en backtracking?

DFS (Depth-First Search) explora en profundidad cada rama del árbol de decisiones antes de retroceder. En backtracking, intentamos todos los movimientos posibles desde una casilla recursivamente hasta encontrar solución o agotar opciones.

### 2. ¿Por qué Warnsdorff es más rápido que Backtracking?

Warnsdorff usa una heurística que reduce drásticamente el espacio de búsqueda. En lugar de probar todos los caminos (8^64 posibilidades), elige inteligentemente el próximo movimiento, resultando en complejidad O(n²) vs O(8^(n²)).

### 3. ¿Cuándo usar Programación Dinámica?

PD es ideal cuando:
- Hay subestructura óptima
- Los subproblemas se superponen
- Se necesita optimización (max/min)
- El espacio de estados es manejable

### 4. ¿Diferencia entre Greedy y Backtracking?

- **Greedy**: Hace elección localmente óptima, no retrocede
- **Backtracking**: Explora todas las opciones, retrocede si falla

### 5. ¿Cuál es mejor algoritmo?

Depende del caso:
- **Tableros pequeños + garantía**: Backtracking
- **Tableros grandes + velocidad**: Warnsdorff
- **Optimización con restricciones**: Programación Dinámica

---

**¡Buena suerte con el coloquio! 🍀**

